<%@page import="Developer_Squad.dao.FoodItemsCrud"%>
<%@page import="Developer_Squad.dto.Restaurant"%>
<%@page import="java.util.ArrayList"%>
<%@page import="Developer_Squad.dto.FoodItems"%>
 ...
 */            color: #fff;
        ...
        .hotel-name {
            font-size: 2.8em;
            margin: 0;
            text-shadow: 2px 2px 4px #333, -2px -2px 4px #666;
        }
...
<body>
	<table border="" cellpadding="8px" cellspacing="3px"
		style="background-color: aquamarine; border-radius: 5px;">

		<%
		Restaurant r = (Restaurant) request.getAttribute("items");
		%>
		<header>		<h1 class="hotel-name"><span class="highlight"><%=r.getName()%></span></h1>
		</header>
		<%
		FoodItemsCrud foodItemsCrud = new FoodItemsCrud();
		List<FoodItems> a = foodItemsCrud.fetch(r.getId());
		for (FoodItems i : a) {
		%>
		<%
		if (i.getName().equalsIgnoreCase("Biryani") && r.getName().equalsIgnoreCase("mehfil")) {
		%>
		<div class="item-lists">
			...
			<div class="item-list">
				...
				<div>
					<h1>
						<span><i><i class="fa-solid fa-indian-rupee-sign"></i></i></span><%=i.getPrice()%></h1>
					<h3>
						...
						<br>
						<button class="addtocart">
							<div class="pretext"><i class="fa-solid fa-trash"></i>
		...
		%>
		<%
		if (i.getName().equalsIgnoreCase("Dum Biryani") && r.getName().equalsIgnoreCase("mehfil")) {
		%>
		<div class="item-lists">
			<div>
				<img
					src="https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcTOjzb5lgvln78HSHc0GFMRG8N5v5lv_xTJUS9fUt4t-ryo6DclRmHG928694YHYqLCoEY&usqp=CAU"
					alt="">
			</div>
			<div class="item-list">
				<div class="item-de">
					...
					<i class="fa-sharp fa-solid fa-star"></i><span><i>3.7</i></span>
					<p class="item-m">
						<i>Dum Biryani served with salan, raita and salad.</i>
					</p>
				</div>
		...
		<%
		if (i.getName().equalsIgnoreCase("Mutton Biryani") && r.getName().equalsIgnoreCase("mehfil")) {
		%>
		<div class="item-lists">
			<div>
				<img
			...
			<div class="item-list">
				<div class="item-de">
					...
					<p class="item-m">
						<i>Special Mutton Biryani ( Mutton Biryani Serves with Extra
							12 Piece of Mutton, Mirchi Ka Salan, Dahi Ka Chutney, Salad).</i>
					</p>
				</div>
				<div>
					<h1>
		...
		<%
		if (i.getName().equalsIgnoreCase("Pizza") && r.getName().equalsIgnoreCase("mehfil")) {
		%>
		<div class="item-lists">
			<div>
				<img
			...
			<div class="item-list">
				<div class="item-de">
					...
					<p class="item-m">
						<i>Pizza topped with our herb-infused signature pan sauce and
							100% mozzarella cheese. A classic treat for all cheese lovers out
							there! </i>
					</p>
				</div>
				<div>
		...
		%>
		<%
		if (i.getName().equalsIgnoreCase("Shawarma") && r.getName().equalsIgnoreCase("mehfil")) {
		%>
		<div class="item-lists">
			<div>
				<img
			...
			<div class="item-list">
				<div class="item-de">
					...
					<i class="fa-sharp fa-solid fa-star"></i><span><i>3.7</i></span>
					<p class="item-m">
						<i>Succulent chicken shawarma wrapped in thin, soft rumali roti.</i>
					</p>
				</div>
		...
		%>
			<%
		if (i.getName().equalsIgnoreCase("sandwich") && r.getName().equalsIgnoreCase("mehfil")) {
		%>
		<div class="item-lists">
			<div>
				<img
			...
			<div class="item-list">
				<div class="item-de">
					...
					<i class="fa-sharp fa-solid fa-star"></i><span><i>3.7</i></span>
					<p class="item-m">
						<i>A no frills chicken sandwich with diced chicken, mayonnaise and lettuce [Energy: 474 kcal, Protein: 38.2g, Carbohydrates: 36.6g, Fiber: 14.9g, Fat: 18.8g].</i>
					</p>
				</div>
		...
		%>
		<%
		if (i.getName().equalsIgnoreCase("Ice Cream") && r.getName().equalsIgnoreCase("mehfil")) {
		%>
		<div class="item-lists">
			<div>
				<img
		...
		%>
		<%
		if (i.getName().equalsIgnoreCase("Orange juice") && r.getName().equalsIgnoreCase("mehfil")) {
		%>
		<div class="item-lists">
			<div>
				<img
			...
			<div class="item-list">
				<div class="item-de">
					<h1 class="item-name"><%=i.getName()%></h1>
					<i class="fa-sharp fa-solid fa-star"></i><span><i>3.5</i></span>
					<p class="item-m">
		...
		%>
		<%
		if (i.getName().equalsIgnoreCase("Veg biryani") && r.getName().equalsIgnoreCase("paradise")) {
		%>
		<div class="item-lists">
			<div>
				<img
		...
		<%
		if (i.getName().equalsIgnoreCase("Burger") && r.getName().equalsIgnoreCase("paradise")) {
		%>
		<div class="item-lists">
			<div>
				<img
		...
		%>
		<%
		if (i.getName().equalsIgnoreCase("Dum Biryani") && r.getName().equalsIgnoreCase("paradise")) {
		%>
		<div class="item-lists">
			<div>
				<img
		...
		%>
		<%
		if (i.getName().equalsIgnoreCase("Noodles") && r.getName().equalsIgnoreCase("paradise")) {
		%>
		<div class="item-lists">
			<div>
				<img
			...
			<div class="item-list">
				<div class="item-de">
					...
					<i class="fa-sharp fa-solid fa-star"></i></i><span><i>4.2</i></span>
					<p class="item-m">
						<i>A delectable Chinese delight packed with flavors, perfect for all noodle enthusiasts.. </i>
					</p>
				</div>
				<div>
					<h1>
		...
		%>
			<%
		if (i.getName().equalsIgnoreCase("cake") && r.getName().equalsIgnoreCase("paradise")) {
		%>
		<div class="item-lists">
			<div>
				<img
		...
		<%-- <tr
			style="color: rgb(1, 49, 22); background-color: rgb(158, 216, 183); text-align: center;">
			<td><%=item.getName()%></td>
			<td><%=item.getPrice()%></td>
			<td><button>
					<a href="update?id=<%=item.getId()%>">Update</a>
				</button></td>
			<td><button>
		...
		<div>
		<div class="buy">
			<button class="sticky-button log">
				<a href="managerlogin.jsp">Logout</a>
			</button>
		</div>
	</div>
		<div style="width: 100%">
			<button style="width: 100%" class="add">
				<a href="addfooditems.jsp">Add New Item</a>
			</button>
		</div>
	</table>
